...
| Arquivo | O que é |
|---|---|
| `public/index.html`, `public/admin.html` | Site público e painel (estáticos, servidos na raiz). |
| `api/state.js` | Função serverless (GET público / POST com senha). |
| `public/bolao.seed.json` | Dados iniciais (committado; servido em `/bolao.seed.json`). |
| `build.js` | Regenera `public/bolao.seed.json` e `public/index.html` dos JSON oficiais. |
| `vercel.json` | Diz ao Vercel: rodar `node build.js` e servir `public/`. |
...
1. **Suba o projeto** no Vercel: **Add New… → Project**, conecte o repositório e clique
   **Deploy**. O `vercel.json` já define tudo (build `node build.js`, saída em `public/`,
   funções em `api/`) — não precisa mexer em Build Command nem Output Directory.
